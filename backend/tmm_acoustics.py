...
class TMMInstrument:
    ...
    def _prepare_phase(self):
        ...
        # Numba fast-path arrays: lossless instruments with no metamaterial
        # actions only. Rebuilt on every prepare so post-construction wiring
        # (e.g. attaching meta_slots then calling _prepare_phase again) stays
        # on the authoritative Python walk; the compiled loop implements pipe,
        # junction2, and hole actions but not pipe_meta / meta_branch.
        has_meta_actions = any(
            a[0] in ("pipe_meta", "meta_branch") for a in self.actions
        )
        if _NUMBA_ENABLED and self.loss_model is None and not has_meta_actions:
            try:
