"""Bell-first sequential optimized with phase_cost (register-aware)."""
import sys, time
import numpy as np
...
# Also check phases at initial positions
initial = [150, 250, 350, 450, 550, 650, 750, 850, 950, 1050, 1100, 1150]
inst0 = make_inst(initial)
print("=== Bell-first sequential: phase check ===")
for i, (n, t) in enumerate(zip(names, targets)):
    wl = SPEED_OF_SOUND / t
    ph = inst0.resonance_phase(wl, chart_str[i])
    print("  %s: target=%6.1fHz phase=%.4f (want=1)" % (n, t, ph))
print("phase_cost=%.6f  freq_cost=%.1fc" % (eval_phase(initial), eval_freq(initial)))

# Optimize
bounds = [(100, 1190)] * 12
for i in range(11):
    bounds[i] = (max(100, initial[i]-200), min(1190, initial[i]+200))
...
dt = time.time() - t0
print("\n=== Optimized ===")
print("phase=%.6f freq=%.1fc (%.0fs)" % (de.fun, eval_freq(de.x), dt))
