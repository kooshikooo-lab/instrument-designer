"""Test: graduated hole sizes (8->14mm) on 12-hole chromatic."""
import sys, math, time
import numpy as np
sys.path.insert(0, 'backend')
from optimizer_global import GlobalFingeringOptimizer

FIXED_REGISTER = [(80.0, 2.5, 3.0)]

targets = [73.416, 77.782, 82.407, 87.307, 92.499, 97.999,
           103.826, 110.000, 116.541, 123.471, 130.813, 138.591, 146.832]
names = ["D2","D#2","E2","F2","F#2","G2","G#2","A2","A#2","B2","C3","C#3","D3"]

N_FREE = 12
N_FIXED = 1

chart = []
for i in range(13):
    row = ["closed"] * (N_FREE + N_FIXED)
    for j in range(i):
        row[j] = "open"
    row[N_FREE] = "closed"
    chart.append(row)

# Graduated hole diameters: 8mm near reed -> 14mm near bell
# (S_hole/S_bore: 10% -> 31% of 25mm bore)
dias = np.linspace(8.0, 14.0, 12).tolist()
lens = [5.0] * 12

print("=== Graduated hole sizes (8->14mm) ===")
for i, d in enumerate(dias):
    print(f"  Hole {i+1}: {d:.1f}mm dia")

# Initial guess from interpolation of 7-hole results
hole_positions = [85, 108, 143, 188, 233, 278, 323, 367, 412, 457, 501, 546]

t0 = time.time()
opt = GlobalFingeringOptimizer(
    targets_hz=targets,
    fingering_chart=chart,
    bore_radius=12.5,
    outer_diameter=37.0,
    hole_diameter=11.0,
    hole_length=5.0,
    hole_diameters=dias,
    hole_lengths=lens,
    closed_top=True,
    n_register=1,
    bore_length=1211.3,
    fixed_holes=FIXED_REGISTER,
)

result = opt.optimize(initial_positions=hole_positions, bounds_per_hole=80, use_de=False, verbose=True)
t1 = time.time()
print(f"\nTime: {t1-t0:.0f}s")
print(f"Result: RMS={result['final_rms_cents']:.2f}c")
positions = result['free_hole_positions']
print(f"Holes: {[f'{p:.0f}' for p in positions]}")
for i, (p, d) in enumerate(zip(positions, dias)):
    print(f"  Hole {i+1}: pos={p:.0f}mm dia={d:.1f}mm")
