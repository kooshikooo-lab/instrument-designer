...
class TestBranchDeleteGuard:
    ...
    def test_feature_delete_content_proven_by_presence(self, monkeypatch):
        # content_preserved() consults real local refs; if opencode/main/desktop
        # exists it likely contains shared history — treat as no-violation if so.
        monkeypatch.setenv("GUARD_BRANCH_ALLOW_DELETE", "")
        violations = guard_branch.check_delete("opencode/does-not-exist/desktop")
        assert isinstance(violations, list)


class TestOriginHead:
    def test_origin_head_audit_clean_after_fix(self):
        # Non-destructive: just ensure the audit function returns a list.
        assert isinstance(guard_branch.audit(), list)
