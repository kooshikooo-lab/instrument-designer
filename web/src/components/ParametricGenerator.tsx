...
const PARAMETRIC_PRESETS: Record<
  string,
  {
    name: string;
    description: string;
...
export default function ParametricGenerator({
  instrumentKey,
  onGenerated,
}: ParametricGeneratorProps) {
  const presetKey =
    instrumentKey && PARAMETRIC_PRESETS[instrumentKey]
      ? instrumentKey
      : "default";
  const preset = PARAMETRIC_PRESETS[presetKey];
  const [params, setParams] = useState<Record<string, number>>(
    Object.fromEntries(
      Object.entries(preset.params).map(([k, v]) => [k, v.default])
    )
  );
    ...
    try {
      ...
      if (presetKey === "recorder") {
        const outer = primitives.cylinder({
          radius: params.boreDiameter / 2 + params.wallThickness,
          height: params.length,
          segments: 64,
        });
        const inner = primitives.cylinder({
        ...
        const outer = primitives.cuboid({
          size: [params.width, params.height, params.length],
        });
        const inner = primitives.cuboid({
          size: [
            params.width - params.wallThickness * 2,
            params.height - params.wallThickness * 2,
          ...
          <div key={key}>
            ...
            <input
              ...
              onChange={(e) =>
                setParams({
                  ...params,
                  [key]: parseFloat(e.target.value) || param.default,
                })
              }
