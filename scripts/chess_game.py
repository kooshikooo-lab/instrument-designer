...
def cmd_challenge(peer_ip, port, match_games=10, time_control="60+0"):
    """Desktop challenges laptop to a match."""
    ...
    for game_num in range(1, match_games + 1):
        ...
        if accept is None:
            print(f"[{_machine_name()}] No acceptance within {CHALLENGE_TIMEOUT}s. Win by forfeit.")
            scores["1-0"] += 1
            continue

        print(f"[{_machine_name()}] Challenge accepted. Playing White.")
...
def cmd_accept(peer_ip, port):
    """Wait for a challenge, accept it, and play Black."""
