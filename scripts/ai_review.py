...
Usage:
    ...
    python scripts/ai_review.py --prompt docs/AI_REVIEW_PROMPT.md \
        --files backend/benchmark_all.py backend/jax_optimizer.py \
...
def call_model(model: str, user_message: str, max_tokens: int = 4000, timeout: int = 300) -> str:
    key = get_api_key()
    headers = {
        "Authorization": f"Bearer {key}",
        "Content-Type": "application/json",
        "HTTP-Referer": os.environ.get("OPENROUTER_REFERER", "https://github.com/kooshikooo-lab/instrument-designer"),
        "X-Title": os.environ.get("OPENROUTER_TITLE", "instrument-designer"),
    }
    payload = {
        "model": model,
        "messages": [
            {"role": "system", "content": "You are a senior Python code reviewer and musical instrument acoustics expert."},
